...
const PUBLIC_ROUTES = { today: "today", sound: "white-noise", vent: "release", joy: "joy" };
const ROUTE_ALIASES = { "white-noise": "sound", release: "vent" };
const GUIDE_SLUGS = ["digital-hug", "anxiety-breathing", "white-noise", "anonymous-venting", "cozy-web-corner"];

const META = {
  en: {
    today: ["Today's Hug | A gentle pause for tired minds", "A tiny emotional pause for anxious, tired, or overstimulated moments."],
    sound: ["White Noise | Calm sounds for sleep, focus, and reset", "Choose a gentle background sound to help yourself slow down."],
    vent: ["Release Pool | Let go of what you don't want to carry", "Write down what feels heavy, privately and gently. No login required."],
  ...
  zh: {
    today: ["今日拥抱｜给疲惫大人的一句温柔话", "当你感到疲惫、焦虑或需要短暂停一下时，来 OASIS 获得一句今日拥抱。"],
    sound: ["白噪音｜睡前、专注和放空时的声音角落", "选择雨声、海浪、森林风或咖啡馆背景音，让自己慢慢安静下来。"],
    vent: ["解压池｜匿名写下此刻不想带走的情绪", "一个不需要登录的轻量解压入口，把压力写下来，然后让它慢慢沉入水底。"],
    joy: ["快乐交换商店 - 匿名交换小开心 | OASIS", "匿名提交一个小小开心，收到陌生人的快乐瞬间。"]
  },
  tc: {
    today: ["今日擁抱｜給疲憊大人的一句溫柔話", "當你感到疲憊、焦慮或需要短暫停一下時，來 OASIS 獲得一句今日擁抱。"],
    sound: ["白噪音｜睡前、專注和放空時的聲音角落", "選擇雨聲、海浪、森林風或咖啡館背景音，讓自己慢慢安靜下來。"],
    vent: ["解壓池｜匿名寫下此刻不想帶走的情緒", "一個不需要登入的輕量解壓入口，把壓力寫下來，然後讓它慢慢沉入水底。"],
...
function normalizeRoute(value) {
  const route = ROUTE_ALIASES[value] || value;
  return ROUTES.has(route) ? route : "today";
}

function publicRouteFor(route) {
  return PUBLIC_ROUTES[normalizeRoute(route)] || "today";
}
...
function originFor(req) {
  ...
  if (!String(host).includes("localhost")) return "https://tiny-joy-oasis.vercel.app";
  return `${proto}://${host}`;
}
...
function structuredData(origin, lang, route, title, description) {
  const publicRoute = publicRouteFor(route);
  return {
    "@context": "https://schema.org",
    "@type": "WebApplication",
    name: "OASIS",
    url: `${origin}/oasis/${lang}/${publicRoute}`,
...
function gaSnippet({ lang, route, guideSlug, sendPageView = true } = {}) {
  const gaId = process.env.NEXT_PUBLIC_GA_ID;
  if (!gaId) return "";
  const siteHost = (() => {
    try {
      return new URL(process.env.NEXT_PUBLIC_SITE_URL || "https://tiny-joy-oasis.vercel.app").hostname;
    } catch (error) {
      return "tiny-joy-oasis.vercel.app";
    }
  })();
  const config = {
  ...
  if (guideSlug) config.guide_slug = guideSlug;
  return `<script>
    (function() {
      var GA_MEASUREMENT_ID = ${JSON.stringify(gaId)};
      var allowedHost = ${JSON.stringify(siteHost)};
      if (!GA_MEASUREMENT_ID || window.location.hostname !== allowedHost) {
        window.oasisAnalyticsReady = false;
        window.oasisTrack = function() {};
        return;
      }
      window.dataLayer = window.dataLayer || [];
      window.gtag = function() { window.dataLayer.push(arguments); };
      window.gtag("js", new Date());
      window.gtag("config", GA_MEASUREMENT_ID, ${JSON.stringify(config)});
      window.oasisAnalyticsReady = true;
      window.oasisTrack = function(eventName, params) {
...
<head>
  ...
  ${gaSnippet({ lang: safeLang, route: "guide", guideSlug: safeSlug })}
  <style>
    ...
    .logo { letter-spacing: 2px; font-weight: 600; color: #334155; text-decoration: none; }
    ...
    h1 { color: #475569; font-size: clamp(28px, 4.4vw, 44px); line-height: 1.35; letter-spacing: 0; font-weight: 400; margin: 26px 0 22px; max-width: 780px; }
    .intro { font-size: 17px; line-height: 1.95; color: #64748b; margin: 0 0 34px; }
    ...
    h2 { color: #475569; font-weight: 500; font-size: 18px; line-height: 1.6; margin: 0 0 10px; }
    p { color: #94a3b8; font-size: 15px; line-height: 1.95; margin: 0; }
...
function renderOasisHtml(req, lang, route) {
  ...
  const publicRoute = publicRouteFor(safeRoute);
  const origin = originFor(req);
  const [title, description] = META[safeLang][safeRoute];
  const canonical = `${origin}/oasis/${safeLang}/${publicRoute}`;
  ...
  const replacements = {
    ...
    "__ALT_EN__": `${origin}/oasis/en/${publicRoute}`,
    "__ALT_ZH__": `${origin}/oasis/zh/${publicRoute}`,
    "__ALT_TC__": `${origin}/oasis/tc/${publicRoute}`,
    "__ALT_DEFAULT__": `${origin}/oasis/en/${publicRoute}`,
    ...
    "__GA_SNIPPET__": gaSnippet({ lang: safeLang, route: safeRoute, sendPageView: false }),
